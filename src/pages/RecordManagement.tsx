...
import { ArrowLeft, Plus, FileText, Lock, Shield, User, Sparkles, AlertTriangle, Cloud, CloudOff, Search } from "lucide-react";
...
const RecordManagement = () => {
  ...
  const [selectedPatientForSummary, setSelectedPatientForSummary] = useState<string | null>(null);
      ...
      <main className="container mx-auto px-4 py-8">
        {anomalies.length > 0 && (
          <Alert className="mb-6 border-destructive/50 bg-destructive/5">
            ...
            <AlertDescription>
              <strong>Anomalies Detected:</strong> {anomalies.length} potential issues found.
        ...
        <div className="flex justify-between items-center mb-6 flex-wrap gap-4">
          <div className="flex items-center gap-2 text-sm bg-primary/5 px-4 py-2 rounded-lg border border-primary/20">
            ...
            <span className="font-medium text-primary">End-to-End Encryption Active</span>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
              <DialogHeader>
                <DialogTitle className="text-2xl font-display">Add Encrypted Medical Record</DialogTitle>
              </DialogHeader>
              <form onSubmit={handleSubmit} className="space-y-4 mt-4">
                ...
                <Button type="submit" className="w-full">Add Encrypted Record</Button>
              </form>
            </DialogContent>
            </Dialog>
          </div>
        ...
        <div className="space-y-6">
          {filteredRecords.map((record) => (
            <Card key={record.id} className="p-6 hover:shadow-lg transition-shadow border-l-4 border-l-primary">
              <div className="flex items-start gap-4">
                <div className="h-14 w-14 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                  <FileText className={`h-7 w-7 ${getRecordTypeColor(record.type)}`} />
                </div>
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2 flex-wrap">
                    <h3 className="font-semibold text-xl text-gov-navy">{record.title}</h3>
                  ...
                  <p className="text-sm text-muted-foreground mb-3">
                    ...
                    <span className="ml-2">{new Date(record.date).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                  </p>
                  ...
                  <div className="bg-secondary/30 p-4 rounded-lg border border-border">
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-2">
                        <Lock className="h-4 w-4 text-primary" />
                        <span className="text-xs font-semibold text-primary">ENCRYPTED CONTENT</span>
                      </div>
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => handleGenerateSummary(record.patientId)}
                  ...
                  {selectedPatientForSummary === record.patientId && aiSummary && (
                    <div className="mt-4 bg-accent/10 p-4 rounded-lg border border-accent/30">
                      <div className="flex items-center gap-2 mb-2">
                        <Sparkles className="h-4 w-4 text-accent" />
                        <span className="text-xs font-semibold text-accent">AI GENERATED SUMMARY</span>
                      </div>
        ...
        {filteredRecords.length === 0 && records.length > 0 && (
          <Card className="text-center py-16 border-2 border-dashed">
            ...
            <p className="text-lg font-medium text-muted-foreground">No records match your search</p>
        ...
        {records.length === 0 && (
          <Card className="text-center py-16 border-2 border-dashed">
            ...
            <p className="text-lg font-medium text-muted-foreground">No medical records yet</p>
            <p className="text-sm text-muted-foreground mt-2">Add your first encrypted medical record to get started</p>
          </Card>
        )}
      </main>
    </div>
